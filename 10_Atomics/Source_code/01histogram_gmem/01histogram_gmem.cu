...
void onDevice(Vector<unsigned int> h_a, Vector<int> h_histo){
    ...
    HANDLER_ERROR_ERR(cudaMemcpy(d_histo.elements, h_histo.elements, ARRAY_BYTES_H, cudaMemcpyHostToDevice));
    ...
    HANDLER_ERROR_ERR( cudaMemcpy( h_histo.elements, d_histo.elements, ARRAY_BYTES_H, cudaMemcpyDeviceToHost ) );
...
void test(Vector<unsigned int> h_a, Vector<int> h_histo, Vector<int> h_test){
    ...
    for(int i = 0; i < SIZE_H; i++){
        //printf(" [%i]  %i | %i \n", i, h_histo.getElement(i), h_test.getElement(i));
