...
__global__ void dotKernel(Lock lock, Vector<float> d_a, Vector<float> d_b, float *d_dotValue ){
    ...
    /*At this point, the packed data is stored in thc cache array for each block created.
    Now, that value needs to be stored in d_dotValue in order to have the final value. It is
    necessary to lock d_dotValue to avoid the threads get the wrong value.
    */

    if (cacheIndex == 0){
        // Wait until we get the lock. Each thread needs to get the lock value before updating
        // d_dotValue
...
void onDevice( Vector<float> h_a, Vector<float> h_b, float &h_dotValue ){
    ...
    HANDLER_ERROR_ERR(cudaMemcpy(d_dotValue, &h_dotValue, sizeof(float), cudaMemcpyHostToDevice));
    ...
    HANDLER_ERROR_ERR(cudaMemcpy(&h_dotValue, d_dotValue, sizeof(float), cudaMemcpyDeviceToHost)); 
