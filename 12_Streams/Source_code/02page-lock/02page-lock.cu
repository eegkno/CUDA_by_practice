...
float cuda_malloc_test( bool up ) {
    ...
    for (int i=0; i<100; i++) {
        if (up){
            HANDLER_ERROR_ERR( cudaMemcpy( d_a, a, ARRAY_BYTES, cudaMemcpyHostToDevice ));
				  
        }else{
            HANDLER_ERROR_ERR(cudaMemcpy( a, d_a, ARRAY_BYTES, cudaMemcpyDeviceToHost ));
...
float cuda_host_alloc_test( bool up ) {
    ...
    HANDLER_ERROR_ERR(cudaHostAlloc( (void**)&a, ARRAY_BYTES, cudaHostAllocDefault ));
    HANDLER_ERROR_ERR(cudaMalloc( (void**)&d_a, ARRAY_BYTES ) );
