...
void onDevice( aos h_inaos[], aos h_outaos[], soa h_insoa, soa h_outsoa,  int iter ){
	...
	HANDLER_ERROR_ERR(cudaMemcpy(d_insoa.a, h_insoa.a, ARRAY_BYTES, cudaMemcpyHostToDevice));
	HANDLER_ERROR_ERR(cudaMemcpy(d_insoa.b, h_insoa.b, ARRAY_BYTES, cudaMemcpyHostToDevice));
	HANDLER_ERROR_ERR(cudaMemcpy(d_insoa.c, h_insoa.c, ARRAY_BYTES, cudaMemcpyHostToDevice));
	HANDLER_ERROR_ERR(cudaMemcpy(d_insoa.d, h_insoa.d, ARRAY_BYTES, cudaMemcpyHostToDevice));
	...
	dim3 ThreadsBlocks( 256 );
	 ...
	 //run the kernel
	kernelNonInterleaved<<<GridBlocks,ThreadsBlocks>>>( d_insoa, d_outsoa, ITER );
    HANDLER_ERROR_MSG("kernel panic!!!");

    // copy data back from the GPU to the CPU (soa)
	HANDLER_ERROR_ERR(cudaMemcpy(h_outsoa.a, d_outsoa.a, ARRAY_BYTES, cudaMemcpyDeviceToHost));	
	HANDLER_ERROR_ERR(cudaMemcpy(h_outsoa.b, d_outsoa.b, ARRAY_BYTES, cudaMemcpyDeviceToHost));	
	HANDLER_ERROR_ERR(cudaMemcpy(h_outsoa.c, d_outsoa.c, ARRAY_BYTES, cudaMemcpyDeviceToHost));	
	HANDLER_ERROR_ERR(cudaMemcpy(h_outsoa.d, d_outsoa.d, ARRAY_BYTES, cudaMemcpyDeviceToHost));			
	...
	HANDLER_ERROR_ERR(cudaMemcpy(d_inaos, h_inaos, ARRAY_BYTES_AOS, cudaMemcpyHostToDevice));
	...
	HANDLER_ERROR_ERR(cudaMemcpy(h_outaos, d_inaos, ARRAY_BYTES_AOS, cudaMemcpyDeviceToHost));	
...
void onHost(aos h_inaos[], aos h_outaos[], soa h_insoa, soa h_outsoa,  int iter ){
